...
compile_apron () {
    ...
    cmake --build . --target llvm -- -j 4 && cmake ..
...
compile_aplas_double () {
    cd $CRABLLVMLOCATION
    mkdir -p $CRABLLVMINSTALL_LOCATION_APLAS_DOUBLE
    mkdir -p build && cd build
    rm -rf crab ../crab
    cmake -DCMAKE_INSTALL_PREFIX=$CRABLLVMINSTALL_LOCATION_APLAS_DOUBLE -DUSE_APRON=ON -DUSE_APLAS=ON -DUSE_APRON_ORIG=OFF -DUSE_HASH=OFF -DUSE_HASH_OPT=OFF -DUSE_APRON_DOUBLE=ON ../
    cmake --build . --target crab && cmake ..
    cmake --build . --target apron && cmake ..
    cmake --build . --target llvm && cmake ..
    cmake --build . --target install
...
compile_hash () {
    cd $CRABLLVMLOCATION
    mkdir -p $CRABLLVMINSTALL_LOCATION_HASH
    mkdir -p build && cd build
    rm -rf crab ../crab
    cmake -DCMAKE_INSTALL_PREFIX=$CRABLLVMINSTALL_LOCATION_HASH -DUSE_APRON=ON -DUSE_APLAS=OFF -DUSE_APRON_ORIG=OFF -DUSE_HASH=ON -DUSE_HASH_OPT=OFF -DUSE_APRON_DOUBLE=OFF ../
    cmake --build . --target crab && cmake ..
    cmake --build . --target apron && cmake ..
    cmake --build . --target llvm && cmake ..
    cmake --build . --target install
...
compile_hash_double () {
    cd $CRABLLVMLOCATION
    mkdir -p $CRABLLVMINSTALL_LOCATION_HASH_DOUBLE
    mkdir -p build && cd build
    rm -rf crab ../crab
    cmake -DCMAKE_INSTALL_PREFIX=$CRABLLVMINSTALL_LOCATION_HASH_DOUBLE -DUSE_APRON=ON -DUSE_APLAS=OFF -DUSE_APRON_ORIG=OFF -DUSE_HASH=ON -DUSE_HASH_OPT=OFF -DUSE_APRON_DOUBLE=ON ../
    cmake --build . --target crab && cmake ..
    cmake --build . --target apron && cmake ..
    cmake --build . --target llvm && cmake ..
    cmake --build . --target install
...
compile_hash_opt () {
    cd $CRABLLVMLOCATION
    mkdir -p $CRABLLVMINSTALL_LOCATION_HASHOPT
    mkdir -p build && cd build
    rm -rf crab ../crab
    cmake -DCMAKE_INSTALL_PREFIX=$CRABLLVMINSTALL_LOCATION_HASHOPT -DUSE_APRON=ON -DUSE_APLAS=OFF -DUSE_APRON_ORIG=OFF -DUSE_HASH=OFF -DUSE_HASH_OPT=ON -DUSE_APRON_DOUBLE=OFF ../
    cmake --build . --target crab && cmake ..
    cmake --build . --target apron && cmake ..
    cmake --build . --target llvm && cmake ..
    cmake --build . --target install
...
compile_hash_opt_double () {
    cd $CRABLLVMLOCATION
    mkdir -p $CRABLLVMINSTALL_LOCATION_HASHOPT_DOUBLE
    mkdir -p build && cd build
    rm -rf crab ../crab
    cmake -DCMAKE_INSTALL_PREFIX=$CRABLLVMINSTALL_LOCATION_HASHOPT_DOUBLE -DUSE_APRON=ON -DUSE_APLAS=OFF -DUSE_APRON_ORIG=OFF -DUSE_HASH=OFF -DUSE_HASH_OPT=ON -DUSE_APRON_DOUBLE=ON ../
    cmake --build . --target crab && cmake ..
    cmake --build . --target apron && cmake ..
    cmake --build . --target llvm && cmake ..
    cmake --build . --target install
